...
(defn divide-until-not-divisible-two [divisor target]
  (prn "DIVIDE-UNTIL-NOT-DIVISIBLE-TWO" divisor target)
  (if (< divisor (Math/sqrt target))
    (if (= (mod target divisor) 0)
      (divide-until-not-divisible-two divisor (/ target 2))
      (divide-until-not-divisible-two (+ divisor 2) target))
    target))

(->>
  (divide-until-not-divisible 2 100)
  (divide-until-not-divisible-two 3)
  (prn))
